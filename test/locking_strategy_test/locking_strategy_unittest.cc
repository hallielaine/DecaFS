...
TEST(LockingTest, SimpleRead) {
  ...
  ASSERT_EQ(std::future_status::ready, rsf.wait_for(base_duration(.5)));
  auto rsr = rsf.get();
...
TEST(LockingTest, SimpleWrite) {
  ...
  ASSERT_EQ(std::future_status::ready, wsf.wait_until(start + base_duration(2.5)));
  ASSERT_EQ(std::future_status::ready, rsf.wait_until(start + base_duration(2.5)));
...
TEST(LockingTest, MultiRead) {
  ...
  auto rs2f = locked_read(&r2, &i, 1, 2, 1,  .5, 1);
  auto rs3f = locked_read(&r3, &i, 1, 3, 1,  1, 1);
  auto rs4f = locked_read(&r4, &i, 1, 4, 1,  1.5, 1);
  ASSERT_EQ(std::future_status::ready, rs1f.wait_until(start + base_duration(1.5)));
  ASSERT_EQ(std::future_status::ready, rs2f.wait_until(start + base_duration(2.5)));
  ASSERT_EQ(std::future_status::ready, rs3f.wait_until(start + base_duration(3.5)));
  ASSERT_EQ(std::future_status::ready, rs4f.wait_until(start + base_duration(4.5)));
  auto end = std::chrono::high_resolution_clock::now();
  EXPECT_GT(end - start, base_duration(3.5));
  auto rs1r = rs1f.get();
  auto rs2r = rs2f.get();
  auto rs3r = rs3f.get();
  auto rs4r = rs4f.get();
