 ...
 * be acquired within *timeout* seconds, or blocks indefinitely if *timeout* is
 * negative.
 *
 ...
 * acquired within *timeout* seconds, or blocks indefinitely if *timeout* is
 * negative.
 *
 * Returns 0 on success, or negative on error.
 */
...
/*
 * Tries to acquire an exclusive metadata lock for a process. Fails if the lock
 * cannot be acquired within *timeout* seconds, or blocks indefinitely if
 * *timeout* is negative.
 *
 * Returns 0 on success, or negative on error.
 */
...
/*
 * Tries to acquire a shared metadata lock for a process. Fails if the lock
 * cannot be acquired within *timeout* seconds, or blocks indefinitely if
 * *timeout* is negative.
 *
 * Returns 0 on success, or negative on error.
...
/*
 * Releases a metadata lock, either exclusive or shared. The lock released is
 * whatever kind of lock the process had on the file. Fails if the lock is not
 * owned.
 *
 * Returns 0 on success, or negative on error.
 */
...
/*
 * Checks whether a process has a shared metadata lock. Specifying a negative
 * value for *user_id* or *proc_id* is like a wildcard, and will return whether
 * any *user_id* or *proc_id* has the lock.
 *
 * Returns positive if the lock is held, 0 if not, or negative on error.
 */
